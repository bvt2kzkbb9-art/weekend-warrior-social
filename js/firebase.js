...
// ⚠️ BEZPIECZEŃSTWO: Wczytaj konfigurację z otoczenia lub firebase-config.js
// NIGDY nie commituj API keys bezpośrednio do kodu źródłowego!

let firebaseConfig = null;

// Próba wczytania z pliku (jeśli istnieje)
try {
  // W lokalnym środowisku: importuj firebase-config.js
  if (typeof window !== 'undefined' && !window.location.hostname.includes('github')) {
    // Development: załaduj z lokalnego pliku
    // importSkrypt('./firebase-config.js');
  }
} catch (err) {
  console.warn('[Firebase] config.js nie znaleziony — konfiguruj ręcznie');
}

// Fallback: pobierz z zmiennych środowiska (GitHub Pages → GitHub Secrets)
if (!firebaseConfig) {
  firebaseConfig = {
    ...
    messagingSenderId: window.FIREBASE_MESSAGING_SENDER_ID || 'YOUR_MESSAGING_ID',
    appId: window.FIREBASE_APP_ID || 'YOUR_APP_ID',
  };

  // Sprawdzenie czy konfiguracja jest placeholder'ami
  const isPlaceholder = Object.values(firebaseConfig).some(v => v.startsWith('YOUR_'));
  if (isPlaceholder) {
    console.error('[Firebase] ❌ Konfiguracja nie jest ustawiona!');
    console.error('[Firebase] Kroki:');
    console.error('  1. Skopiuj firebase-config-template.js → firebase-config.js');
    console.error('  2. Uzupełnij danymi z Firebase Console');
    console.error('  3. Dodaj firebase-config.js do .gitignore');
